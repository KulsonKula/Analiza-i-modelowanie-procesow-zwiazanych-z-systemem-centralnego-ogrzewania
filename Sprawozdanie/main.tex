...
Zasobnik CWU został wyposażony w sterownik dwupołożeniowy z pętlą histerezy, co oznaczało, że mechanizm grzewczy operował w dwóch stanach: wysokim i niskim Zmiana między tymi stanami następowała w chwili, gdy temperatura drugiej warstwy przekraczała temperaturę zadaną o określoną wartość delta.\\

W celu uwzględnienia korelacji tygodniowej oraz sezonowej w analizie przepływu wody, zdecydowano się na wybór modelu typu C wraz z dodatkowym parametrem. Symulacje przepływu wody zostały przeprowadzone dla wtorkowego lata. W ramach udoskonalenia modelu, podjęto decyzję o modyfikacji prognoz dotyczących zużycia wody. Wszystkie wartości ujemne zostały przekształcone na zero, ze względu na ich nierealność w kontekście pomiaru zużycia wody.
...
W trakcie przeprowadzonych badań temperatura zadana była ustawiona na wartość X, natomiast czułość pętli histerezy została dostosowana do wartości Y.
\begin{figure}[!h]
  \centering
...
\end{itemize}
W kontekście sterowania dwupołożeniowego, funkcja kosztów pełni istotną rolę, definiując, w jakim stopniu piec był w stanie wysokim i niskim przez określony okres czasu, dzięki czemu może być także zdefiniowana jako procent udziału czas w którym piec był w stanie wysokim wymnożonym przez jego moc. W kontekście sterowania dwupołożeniowego, kluczowymi parametrami tego systemu są temperatura oczekiwana oraz czułość. Te dwa czynniki mają istotny wpływ na dynamikę pieca i efektywność procesu grzewczego. Wiedza ta daje nam możliwość badania charakterystyki zmian funckji celu w zależności od tych dwóch parametrów:
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.5\textwidth]{img/G_3d.png}
  \caption{Wartośc funkcji kosztów w zależności od temepratury zadanej oraz czułości}
  \label{fig:etykieta-rysunku}
\end{figure}
...
\begin{equation}
  J = \int(T_{wo} - T_{\text{wym}} - \delta)\cdot H(T_{wo} - T_{\text{wym}} - \delta_t) + (T_{\text{wym}} - T_{wo} - \delta_t) \cdot H(T_{\text{wym}} - T_{wo} - \delta) \, dt
\end{equation}
% \begin{figure}[!h]
%   \centering
%   \includegraphics[width=0.8\textwidth]{img/J_wygląd.png}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.6\textwidth]{img/J_3d.png}
  \caption{Wartośc funkcji komfortu w zależności od temepratury zadanej oraz czułości}
  \label{fig:etykieta-rysunku}
\end{figure}
