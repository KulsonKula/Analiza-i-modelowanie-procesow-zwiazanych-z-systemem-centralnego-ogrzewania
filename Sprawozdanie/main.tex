...
\chapter{Modelowanie temperatury wody użytkowej (?)}
\label{ch:05}
...
Zasobnik CWU został wyposażony w regulator proporcjonalno, który został starannie dostrojony w celu osiągnięcia liniowego działania w zakresie czułości. W ramach tego zakresu regulator proporcjonalny reaguje proporcjonalnie do różnicy pomiędzy wartością zadaną a rzeczywistą wartością procesu. Poza tym określonym zakresem czułości, regulator przyjmuje stałą wartość równą 1 lub 0. Działanie tego regulatora można przedstawić wzorem:
\begin{equation}
  u(t) = \begin{cases}
    1                                                  & \text{jeśli } t_{wo} \geq t_{\text{wym}} + \delta                                          \\
    -\frac{0.5}{\delta}(t_{wo} - t_{\text{wym}}) + 0.5 & \text{jeśli } t_{wo} < t_{\text{wym}} + \delta \text{ i } t_{wo} > t_{\text{wym}} - \delta \\
    0                                                  & \text{jeśli } t_{wo} \leq t_{\text{wym}} - \delta
  \end{cases}
\end{equation}

\begin{itemize}
  \item $t_{\text{wym}}$ - Temepratura zadana
  \item $t_{\text{wo}}$ - Temperatura wyjściowa
  \item $\delta$ - Czułość
...
W trakcie przeprowadzonych badań temperatura zadana była ustawiona na wartość $50^\circ C$, natomiast czułość regualtora została dostosowana do wartości 4.
\begin{figure}[!h]
  \centering
